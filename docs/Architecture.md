...
> **Current vs. future — how to read this document.** §1-19 describe Featuresmith's architecture as a converged whole — the shape every part of the system is designed toward. Not all of it is built yet. As of v0.2.0 (shipped, frozen): the Review Engine, ML Readiness Score, Leakage Detection, Dataset Diff (standalone), and the CSV/Excel/Parquet/DataFrame connectors are real, tested, and in production use — all with static, in-repo registration, no `entry_points` discovery yet (`Architecture.md` §25.1). **Not built in v0.2.0**, referenced below as target design: the AI Layer/`AIProvider`/chat (§7) — Future, applicable when the AI layer is introduced in v1.x; `.featuresmith.yml` configuration — Future, applicable when the config system is introduced; the Recommendation Engine (§8) — Future, v0.4; the Dashboard (§14) — Future, v0.3; the VS Code extension — Future, v2.0+; entry-point plugin discovery for any category — Future, introduced per category on demonstrated need (§25.1); and the entire Plan/Export/Contract lifecycle (§20) — Future, v0.4-v0.6, explicitly marked "Design Only — Not Yet Implemented" in its own spec (`features/Dataset-Contracts-And-Planning.md`). `implementation/IMPLEMENTATION_STATUS.md` is the authoritative, current record of what's actually shipped; where this document and that tracker seem to disagree on status, the tracker wins. §21 below is this document's own detailed v0.2.0-vs-design assessment.
