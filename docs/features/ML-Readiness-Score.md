...
> **Status: Implemented.** The scoring framework (dimensions, registry, aggregator, schema), 7 effective dimensions, Score Adapter integration with Review Engine, `fs.review()` score attachment, CLI `--no-score`, and `fs.score()` accessor are implemented. `FeatureQualityDimension` is active. `DistributionHealthDimension` reads `basic_statistics` as a stub (DistributionReviewer/OutlierReviewer not yet implemented). `ClassBalanceDimension` is omitted pending minority-class detector implementation. `ConsistencyDimension` and `DataQualityDimension` are reconciled per spec §7.1 (cardinality double-count eliminated). CLI `--fail-below`/`--fail-below-dimension` and `.featuresmith.yml` weight configuration remain future work. The active scoring formula version is `0.3.0`.
